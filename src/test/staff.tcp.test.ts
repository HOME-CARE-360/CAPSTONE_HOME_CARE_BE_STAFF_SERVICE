...
class Logger {
  ...
  static logResponse<T>(operation: string, result: TCPResponse<T>): void {
    ...
    if (isError) {
      console.error(
        `❌ [${timestamp}] ${operation} Failed (${result.statusCode}):`,
      );
      console.error(
        "Error:",
...
class StaffApiClient {
  ...
  static async getInspectionReportsByStaff(
    staffId: number,
  ): Promise<TCPResponse | null> {
    if (!staffId || staffId <= 0) {
      throw new Error("Valid staffId is required");
  ...
  static async getBookingWorkflow(
    staffId: number,
    bookingId: number,
  ): Promise<TCPResponse | null> {
    if (!staffId || staffId <= 0) {
...
class TestRunner {
  ...
  async runBasicTests(): Promise<void> {
    ...
    try {
      ...
      await StaffApiClient.getBookingDetail(
  ...
  async runWorkflowTests(): Promise<void> {
    ...
    try {
      await StaffApiClient.getBookingWorkflow(
        this.config.staffId,
        this.config.bookingId,
      );
    } catch (error) {
